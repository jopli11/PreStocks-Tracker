...
export default function TokenTicker() {
  ...
  // Create a block: [Tokens]
  const tickerBlock = filteredTokens.map(t => ({ type: 'token' as const, ...t }));

  // Duplicate for seamless loop
  const tickerItems = [...tickerBlock, ...tickerBlock, ...tickerBlock];
      ...
      <div className="flex-1 overflow-hidden relative h-full">
        <div className="animate-ticker flex items-center h-full">
          {tickerItems.map((item, idx) => {
            ...
            return (
              <div key={`${item.target?.key}-${idx}`} className="flex items-center">
                <div className="ticker-item gap-6">
                  {/* Logo & Basic Info */}
                  <div className="flex items-center gap-2">
                    <div className="w-7 h-7 rounded-full bg-black/5 border border-black/10 p-0.5 overflow-hidden shrink-0 relative">
                      {item.data?.image && (
                        <Image 
                          src={item.data.image} 
                          alt={item.target?.key || 'Token'} 
                          fill
